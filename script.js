...
const STORAGE_KEYS = {
  balance: "velvetRougeRoulette.v2.balance",
  bankrupt: "velvetRougeRoulette.v2.bankrupt",
};
...
function loadSavedBalance() {
  if (localStorage.getItem(STORAGE_KEYS.bankrupt) === "true") {
    return 0;
  }

  const savedBalance = Number(localStorage.getItem(STORAGE_KEYS.balance));
  if (Number.isFinite(savedBalance) && savedBalance >= 0) {
    return savedBalance;
  }

  localStorage.setItem(STORAGE_KEYS.balance, String(STARTING_BALANCE));
...
function lockOutPlayer() {
  balance = 0;
  localStorage.setItem(STORAGE_KEYS.bankrupt, "true");
  localStorage.setItem(STORAGE_KEYS.balance, "0");
  updateBalance();
  setControlsDisabled(true);
  resetButtonEl.hidden = true;
  resultLabelEl.textContent = "END";
  setDealerState("lose", "is-lose");
  setDealerMessage("所持金がなくなりました。当サロンでのプレイは終了です。リロードしても再開できません。");
}
